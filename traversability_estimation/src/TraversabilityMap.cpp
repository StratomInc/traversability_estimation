...
{
TraversabilityMap::TraversabilityMap(const rclcpp::Node::SharedPtr node)
  : node_(node)
...
{
  RCLCPP_INFO(node_->get_logger(), "Traversability Map started.");

  readParameters();
  traversabilityMapPublisher_ =
      node_->create_publisher<grid_map_msgs::msg::GridMap>("traversability_map", 1);
  footprintPublisher_ =
      node_->create_publisher<geometry_msgs::msg::PolygonStamped>("footprint_polygon", 1);
  untraversablePolygonPublisher_ =
      node_->create_publisher<geometry_msgs::msg::PolygonStamped>("untraversable_polygon", 1);
}
...
void TraversabilityMap::createLayers(bool useRawMap)
{
...
bool TraversabilityMap::readParameters()
{
  ...
  footprint = node_->declare_parameter("footprint.footprint_polygon", "");
  if (!nav2_costmap_2d::makeFootprintFromString(footprint, footprintPoints_))
  {
    RCLCPP_WARN(node_->get_logger(), "Footprint polygon is invalid.");
    footprintPoints_.clear();
  }

  mapFrameId_ = node_->declare_parameter("map_frame_id", "map");
  traversabilityDefaultReadAtInit_ =
      node_->declare_parameter("footprint.traversability_default", 0.5);
  // Safety check
  ...
  checkForRoughness_ = node_->declare_parameter("footprint.verify_roughness_footprint", false);
  checkRobotInclination_ = node_->declare_parameter("footprint.check_robot_inclination", false);
  maxGapWidth_ = node_->declare_parameter("max_gap_width", 0.3);
  criticalStepHeight_ = node_->declare_parameter(
      "critical_step_height", 0.12);  // Same value as critical_value in params stepFilter (TODO to
                                      // refactor and reference that param)

  // Configure filter chain
  if (!filter_chain_.configure("traversability_map_filters", node_->get_node_logging_interface(),
...
bool TraversabilityMap::setElevationMap(const grid_map_msgs::msg::GridMap& msg)
{
  if (getMapFrameId() != msg.header.frame_id)
  {
    RCLCPP_ERROR(node_->get_logger(),
                 "Received elevation map has frame_id = '%s', but an elevation map with frame_id = "
                 "'%s' is expected.",
  ...
  for (auto& layer : elevationMapLayers_)
  {
    if (!elevationMap.exists(layer))
    {
      RCLCPP_WARN(node_->get_logger(),
                  "Traversability Map: Can't set elevation map because there is no layer %s.",
                  layer.c_str());
      return false;
    }
  }
...
bool TraversabilityMap::setTraversabilityMap(const grid_map_msgs::msg::GridMap& msg)
{
  ...
  for (auto& layer : traversabilityMapLayers_)
  {
    if (!traversabilityMap.exists(layer))
    {
      RCLCPP_WARN(
          node_->get_logger(),
          "Traversability Map: Can't set traversability map because there exists no layer %s.",
          layer.c_str());
      return false;
    }
  }
...
{
  std::shared_ptr<grid_map_msgs::msg::GridMap> mapMessage;
  boost::recursive_mutex::scoped_lock scopedLockForTraversabilityMap(traversabilityMapMutex_);
  ...
  {
    traversabilityMapCopy.add("uncertainty_range", traversabilityMapCopy.get("upper_bound") -
                                                       traversabilityMapCopy.get("lower_bound"));
  }
...
{
  boost::recursive_mutex::scoped_lock scopedLockForTraversabilityMap(traversabilityMapMutex_);
  if (traversabilityMap_.exists("step_footprint"))
    traversabilityMap_.clear("step_footprint");
  if (traversabilityMap_.exists("slope_footprint"))
    traversabilityMap_.clear("slope_footprint");
  if (traversabilityMap_.exists("traversability_footprint"))
    traversabilityMap_.clear("traversability_footprint");
}
...
bool TraversabilityMap::computeTraversability()
{
  ...
  if (elevationMapInitialized_)
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(), "Traversability Estimation: Could not update the filter "
                                        "chain! No traversability "
                                        "computed!");
      traversabilityMapInitialized_ = false;
      return false;
    }
  ...
  {
    RCLCPP_ERROR(node_->get_logger(),
                 "Traversability Estimation: Elevation map is not initialized!");
    traversabilityMapInitialized_ = false;
    return false;
  }
  traversabilityMapInitialized_ = true;
  ...
  if (checkForRoughness_)
    traversabilityMapCopy.add("roughness_footprint");
...
bool TraversabilityMap::traversabilityFootprint(double footprintYaw)
{
  ...
  RCLCPP_DEBUG_STREAM(node_->get_logger(), "footprint yaw: " << footprintYaw);
  // Compute Orientation
  ...
  for (grid_map::GridMapIterator iterator(traversabilityMap_); !iterator.isPastEnd(); ++iterator)
  {
    ...
    grid_map::Position3 positionToVertex, positionToVertexTransformedX,
        positionToVertexTransformedRot;
  ...
  RCLCPP_INFO(node_->get_logger(), "Traversability of footprint has been computed in %f s.",
              (node_->now().seconds() - start.seconds()));
  return true;
}
...
bool TraversabilityMap::checkFootprintPath(
    const traversability_msgs::msg::FootprintPath::SharedPtr path,
    traversability_msgs::msg::TraversabilityResult::SharedPtr result, const bool publishPolygons)
{
  bool successfullyCheckedFootprint;
  if (!traversabilityMapInitialized_)
  {
    RCLCPP_WARN(node_->get_logger(),
                "Traversability Estimation: check Footprint path: Traversability map not "
                "yet initialized.");
  ...
  if (arraySize == 0)
  {
    RCLCPP_WARN(node_->get_logger(), "Traversability Estimation: This path has no poses to check!");
    result->is_safe = static_cast<unsigned char>(false);
    return false;
  }
...
bool TraversabilityMap::checkCircularFootprintPath(
    const traversability_msgs::msg::FootprintPath::SharedPtr path, const bool publishPolygons,
    traversability_msgs::msg::TraversabilityResult::SharedPtr result)
{
  ...
  for (int i = 0; i < arraySize; i++)
  {
    ...
    if (arraySize == 1)
    {
      ...
      bool pathIsTraversable = isTraversable(end, radius + offset, computeUntraversablePolygon,
                                             traversability, untraversablePolygon, radius);
      if (publishPolygons)
      {
        grid_map::Polygon polygon = grid_map::Polygon::fromCircle(end, radius + offset);
        polygon.setFrameId(getMapFrameId());
        polygon.setTimestamp(node_->now().nanoseconds());
    ...
    if (arraySize > 1 && i > 0)
    {
      ...
      for (grid_map::LineIterator lineIterator(traversabilityMap_, endIndex, startIndex);
           !lineIterator.isPastEnd(); ++lineIterator)
      {
        grid_map::Position center;
        traversabilityMap_.getPosition(*lineIterator, center);
        pathIsTraversable =
            pathIsTraversable &&
            isTraversable(center, radius + offset, computeUntraversablePolygon, traversabilityTemp,
                          auxiliaryUntraversablePolygon, radius);

        if (publishPolygons && computeUntraversablePolygon &&
            auxiliaryUntraversablePolygon.nVertices() > 0)
        {
          untraversablePolygon =
              grid_map::Polygon::convexHull(untraversablePolygon, auxiliaryUntraversablePolygon);
        }
      ...
      if (pathIsTraversable)
      {
        ...
        if (i > 1)
        {
          ...
          result->traversability =
              (lengthSegment * traversability + lengthPreviousPath * result->traversability) /
              lengthPath;
        }
        else
        {
...
bool TraversabilityMap::checkPolygonalFootprintPath(
    const traversability_msgs::msg::FootprintPath::SharedPtr path, const bool publishPolygons,
    traversability_msgs::msg::TraversabilityResult::SharedPtr result)
{
  grid_map::Position start, end;
  ...
  polygon1.setTimestamp(node_->now().nanoseconds());
  polygon2 = polygon1;
  for (int i = 0; i < arraySize; i++)
  {
    polygon1 = polygon2;
    ...
    if (arraySize == 1)
    {
      ...
      bool pathIsTraversable =
          isTraversable(polygon, computeUntraversablePolygon, traversability, untraversablePolygon);
    ...
    if (arraySize > 1 && i > 0)
    {
      ...
      if (i > 1)
      {
        ...
        result->traversability =
            (areaPolygon * traversability + areaPrevious * result->traversability) / result->area;
      }
      else
      {
        result->area = polygon.getArea();
...
bool TraversabilityMap::isTraversable(const grid_map::Polygon& polygon,
                                      const bool& computeUntraversablePolygon,
                                      double& traversability,
                                      grid_map::Polygon& untraversablePolygon)
{
  ...
  if (pathIsTraversable)
  {
    ...
    if (nCells == 0)
    {
      RCLCPP_DEBUG(node_->get_logger(),
                   "TraversabilityMap: isTraversable: No cells within polygon.");
      traversability = traversabilityDefault_;
  ...
  if (computeUntraversablePolygon)
  {
    ...
    {
      untraversablePolygon =
          grid_map::Polygon::monotoneChainConvexHullOfPoints(untraversablePositions);
    }
    untraversablePolygon.setFrameId(getMapFrameId());
    untraversablePolygon.setTimestamp(node_->now().nanoseconds());
  }

  return pathIsTraversable;
}

bool TraversabilityMap::isTraversable(const grid_map::Position& center, const double& radiusMax,
                                      double& traversability, const double& radiusMin)
{
  const bool computeUntraversablePolygon = false;
  grid_map::Polygon untraversablePolygon;
  return isTraversable(center, radiusMax, computeUntraversablePolygon, traversability,
                       untraversablePolygon, radiusMin);
}
           ...
           !iterator.isPastEnd() && !traversableRadiusBiggerMinRadius; ++iterator)
      {
        ...
        if (!currentPositionIsTraversale)
        {
          ...
          else
          {
            ...
            else if (circleIsTraversable)
            {  // if circleIsTraversable is not changed by any previous loop
              auto factor =
                  ((untraversableRadius - radiusMin) / (radiusMax - radiusMin) + 1.0) / 2.0;
              traversability *= factor / nCells;
              traversabilityMap_.at("traversability_footprint", indexCenter) =
                  static_cast<float>(traversability);
...
bool TraversabilityMap::checkInclination(const grid_map::Position& start,
                                         const grid_map::Position& end)
{
  boost::recursive_mutex::scoped_lock scopedLockForTraversabilityMap(traversabilityMapMutex_);
  if (end == start)
  {
    if (traversabilityMap_.atPosition(robotSlopeType_, start) == 0.0)
      return false;
  }
  else
  {
    ...
    traversabilityMap_.getIndex(start, startIndex);
    traversabilityMap_.getIndex(end, endIndex);
    for (grid_map::LineIterator lineIterator(traversabilityMap_, startIndex, endIndex);
         !lineIterator.isPastEnd(); ++lineIterator)
    {
      if (!traversabilityMap_.isValid(*lineIterator, robotSlopeType_))
        continue;
      if (traversabilityMap_.at(robotSlopeType_, *lineIterator) == 0.0)
        return false;
    }
  }
  return true;
...
bool TraversabilityMap::checkForStep(const grid_map::Index& indexStep)
{
  ...
  if (traversabilityMap_.at(stepType_, indexStep) == 0.0)
  {
    if (!traversabilityMap_.isValid(indexStep, "step_footprint"))
    {
      ...
      if (indices.empty())
        indices.push_back(indexStep);
      for (auto& index : indices)
      {
        grid_map::Length subMapLength(2.5 * traversabilityMap_.getResolution(),
                                      2.5 * traversabilityMap_.getResolution());
        ...
        {
          RCLCPP_WARN(node_->get_logger(),
                      "Traversability map: Check for step window could not retrieve submap.");
          traversabilityMap_.at("step_footprint", indexStep) = 0.0;
          return false;
        }
        height = traversabilityMap_.at("elevation", index);
        for (grid_map::GridMapIterator subMapIterator(subMap); !subMapIterator.isPastEnd();
             ++subMapIterator)
        {
          if (subMap.at(stepType_, *subMapIterator) == 0.0 &&
              subMap.at("elevation", *subMapIterator) < height - criticalStepHeight_)
          {
            ...
            {
              if (toCenter.dot(vec) < 0.0)
                continue;
            }
            pos = subMapPos + vec;
            while ((pos - subMapPos + vec).norm() < maxGapWidth_ &&
                   traversabilityMap_.isInside(pos + vec))
              pos += vec;
            grid_map::Index endIndex;
            traversabilityMap_.getIndex(pos, endIndex);
            bool gapStart = false;
            bool gapEnd = false;
            for (grid_map::LineIterator lineIterator(traversabilityMap_, index, endIndex);
                 !lineIterator.isPastEnd(); ++lineIterator)
            {
...
bool TraversabilityMap::checkForSlope(const grid_map::Index& index)
{
  ...
  if (traversabilityMap_.at(slopeType_, index) == 0.0)
  {
    ...
    {
      double windowRadius =
          3.0 * traversabilityMap_.getResolution();  // TODO: read this as a parameter?
      double criticalLength = maxGapWidth_ / 3.0;
      int nSlopesCritical =
          floor(2 * windowRadius * criticalLength / pow(traversabilityMap_.getResolution(), 2));
  ...
  if (traversabilityMap_.at(roughnessType_, index) == 0.0)
  {
    if (!traversabilityMap_.isValid(index, "roughness_footprint"))
    {
      double windowRadius =
          3.0 * traversabilityMap_.getResolution();  // TODO: read this as a parameter?
      double criticalLength = maxGapWidth_ / 3.0;
      int nRoughnessCritical =
          floor(1.5 * windowRadius * criticalLength / pow(traversabilityMap_.getResolution(), 2));
...
void TraversabilityMap::publishUntraversablePolygon(const grid_map::Polygon& untraversablePolygon,
                                                    double zPosition)
{
  if (untraversablePolygon.nVertices() == 0)
  {
    return;
  }
  geometry_msgs::msg::PolygonStamped polygonMsg;
...
double TraversabilityMap::boundTraversabilityValue(const double& traversabilityValue) const
{
  ...
  {
    RCLCPP_WARN(node_->get_logger(),
                "Passed traversability value (%f) is higher than max allowed value (%f). It is set "
                "equal to the max.",
                traversabilityValue, traversabilityMaxValue);
    return traversabilityMaxValue;
  }
  if (traversabilityValue < traversabilityMinValue)
  {
    RCLCPP_WARN(node_->get_logger(),
                "Passed traversability value (%f) is lower than min allowed value (%f). It is set "
                "equal to the min.",
                traversabilityValue, traversabilityMinValue);
...
bool TraversabilityMap::mapHasValidTraversabilityAt(double x, double y) const
{
  ...
  if (!indexObtained)
  {
    RCLCPP_ERROR(
        node_->get_logger(),
        "It was not possible to get index of the position (%f, %f) in the current grid_map "
        "representation of the traversability map.",
        x, y);
    return false;
  }
