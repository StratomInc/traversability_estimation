...
public:
   ...
   * Constructor.
   */
  TraversabilityMap(const rclcpp::Node::SharedPtr node);
   ...
   * @param[in] publishPolygons says if checked polygon and untraversable polygon should be computed
   * and published.
   * @param[out] result the traversability result.
   * @return true if successful.
   */
  bool checkFootprintPath(const traversability_msgs::msg::FootprintPath::SharedPtr path,
  ...
  /*!
   * Restores the default traversability value of unknown regions in the map, which was read during
   * initialization .
   */
   ...
   */
  void createLayers(bool useRawMap);
...
private:
  ...
  /*!
   * Gets the traversability value of the submap defined by the polygon. Is true if the whole
   * polygon is traversable.
   * @param[in] polygon polygon that defines submap of the traversability map.
   * @param[in] computeUntraversablePolygon true if untraversable polygon within submap checked for
   * traversability should be computed.
   * @param[out] traversability traversability value of submap defined by the polygon, the
   * traversability is the mean of each cell within the polygon.
   ...
   */
  bool isTraversable(const grid_map::Polygon& polygon, const bool& computeUntraversablePolygon,
                     double& traversability, grid_map::Polygon& untraversablePolygon);
   ...
   * @param[out] untraversablePolygon untraversable polygon within area checked for traversability.
   * @param[in] radiusMin if set (not zero), footprint inflation is applied and radiusMin is the
   * minimum valid radius of the footprint.
  ...
  bool isTraversable(const grid_map::Position& center, const double& radiusMax,
                     const bool& computeUntraversablePolygon, double& traversability,
   ...
   * @param[in] zPosition height of the polygon.
   */
  void publishUntraversablePolygon(const grid_map::Polygon& untraversablePolygon,
   ...
   * @return true if successful.
   */
  bool checkCircularFootprintPath(const traversability_msgs::msg::FootprintPath::SharedPtr path,
                                  const bool publishPolygons,
                                  traversability_msgs::msg::TraversabilityResult::SharedPtr result);
   ...
   * @return true if successful.
   */
  bool checkPolygonalFootprintPath(const traversability_msgs::msg::FootprintPath::SharedPtr path,
                                   const bool publishPolygons,
                                   traversability_msgs::msg::TraversabilityResult::SharedPtr result);
  ...
  std::vector<geometry_msgs::msg::Point> footprintPoints_;
