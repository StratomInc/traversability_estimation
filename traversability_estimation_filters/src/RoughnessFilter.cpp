...
bool RoughnessFilter<T>::configure()
{
  ...
  {
    RCLCPP_ERROR(logger_, "RoughnessFilter did not find param critical_value");
    return false;
  }

  if (criticalValue_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "Critical roughness must be greater than zero");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Critical roughness = %f", criticalValue_);

  if (!FilterBase<T>::getParam(std::string("estimation_radius"), estimationRadius_))
  {
    RCLCPP_ERROR(logger_, "RoughnessFilter did not find param estimation_radius");
    return false;
  }

  if (estimationRadius_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "Roughness estimation radius must be greater than zero");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Roughness estimation radius = %f", estimationRadius_);

  if (!FilterBase<T>::getParam(std::string("map_type"), type_))
  {
    RCLCPP_ERROR(logger_, "RoughnessFilter did not find param map_type");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Roughness map type = %s", type_.c_str());
...
bool RoughnessFilter<T>::update(const T& mapIn, T& mapOut)
{
  ...
  for (GridMapIterator iterator(mapOut); !iterator.isPastEnd(); ++iterator)
  {
    ...
    if (!mapOut.isValid(*iterator, "surface_normal_x"))
      continue;
    ...
    {
      double dist =
          normalX * points(0, i) + normalY * points(1, i) + normalZ * points(2, i) - planeParameter;
      sum += pow(dist, 2);
    }
    ...
    if (roughness > roughnessMax)
      roughnessMax = roughness;
  }

  RCLCPP_DEBUG(logger_, "roughness max = %f", roughnessMax);
...
PLUGINLIB_EXPORT_CLASS(filters::RoughnessFilter<grid_map::GridMap>,
