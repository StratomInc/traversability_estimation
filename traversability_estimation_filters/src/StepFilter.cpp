...
StepFilter<T>::StepFilter()
  ...
  , logger_(rclcpp::get_logger("step_filter"))
{
}
...
bool StepFilter<T>::configure()
{
  ...
  {
    RCLCPP_ERROR(logger_, "Step filter did not find param critical_value.");
    return false;
  }

  if (criticalValue_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "Critical step height must be greater than zero.");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Critical step height = %f.", criticalValue_);

  if (!FilterBase<T>::getParam(std::string("first_window_radius"), firstWindowRadius_))
  {
    RCLCPP_ERROR(logger_, "Step filter did not find param 'first_window_radius'.");
    return false;
  }

  if (firstWindowRadius_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "'first_window_radius' must be greater than zero.");
    return false;
  }

  RCLCPP_DEBUG(logger_, "First window radius of step filter = %f.", firstWindowRadius_);

  if (!FilterBase<T>::getParam(std::string("second_window_radius"), secondWindowRadius_))
  {
    RCLCPP_ERROR(logger_, "Step filter did not find param 'second_window_radius'.");
    return false;
  }

  if (secondWindowRadius_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "'second_window_radius' must be greater than zero.");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Second window radius of step filter = %f.", secondWindowRadius_);

  if (!FilterBase<T>::getParam(std::string("critical_cell_number"), nCellCritical_))
  {
    RCLCPP_ERROR(logger_, "Step filter did not find param 'critical_cell_number'.");
    return false;
  }

  if (nCellCritical_ <= 0)
  {
    RCLCPP_ERROR(logger_, "'critical_cell_number' must be greater than zero.");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Number of critical cells of step filter = %d.", nCellCritical_);

  if (!FilterBase<T>::getParam(std::string("map_type"), type_))
  {
    RCLCPP_ERROR(logger_, "Step filter did not find param map_type.");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Step map type = %s.", type_.c_str());
