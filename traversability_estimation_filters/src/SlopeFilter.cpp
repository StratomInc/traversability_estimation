...
bool SlopeFilter<T>::configure()
{
  ...
  {
    RCLCPP_ERROR(logger_, "SlopeFilter did not find param critical_value");
    return false;
  }

  if (criticalValue_ > M_PI_2 || criticalValue_ < 0.0)
  {
    RCLCPP_ERROR(logger_, "Critical slope must be in the interval [0, PI/2]");
    return false;
  }

  RCLCPP_DEBUG(logger_, "critical Slope = %f", criticalValue_);

  if (!FilterBase<T>::getParam(std::string("map_type"), type_))
  {
    RCLCPP_ERROR(logger_, "SlopeFilter did not find param map_type");
    return false;
  }

  RCLCPP_DEBUG(logger_, "Slope map type = %s", type_.c_str());
...
bool SlopeFilter<T>::update(const T& mapIn, T& mapOut)
{
  ...
  for (GridMapIterator iterator(mapOut); !iterator.isPastEnd(); ++iterator)
  {
    ...
    if (!mapOut.isValid(*iterator, "surface_normal_z"))
      continue;
  ...
  RCLCPP_DEBUG(logger_, "slope max = %f", slopeMax);
